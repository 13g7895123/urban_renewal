...
# 檢查環境參數
if [[ "$TARGET_ENV" != "production" && "$TARGET_ENV" != "dev" ]]; then
    echo -e "${RED}❌ 錯誤：不支援的環境 '$TARGET_ENV'${NC}"
    echo -e "${YELLOW}支援的環境：production, dev${NC}"
    exit 1
fi

echo -e "${GREEN}📦 目標環境：${TARGET_ENV}${NC}"
echo ""

# 檢查環境配置檔案
ENV_FILE="docker/.env.$TARGET_ENV"
if [ ! -f "$ENV_FILE" ]; then
...
cat > backend/.env << EOF
# Auto-generated from docker/.env.$TARGET_ENV
...
COMPOSE_FILE="docker/docker-compose.$TARGET_ENV.yml"
