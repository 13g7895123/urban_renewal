...
#### 4.2 聯絡資訊
- **理事長姓名**: 文字輸入
  - **自動帶入**: 選擇更新會後，自動填入該更新會的理事長姓名
  - **資料來源**: `selectedUrbanRenewal.chairman_name`
  - **說明**: 可手動修改自動帶入的理事長姓名
- **聯絡人姓名**: 文字輸入
- **聯絡人電話**: 文字輸入
- **附件**: 文字輸入
...
#### 3. 匯出會議通知（僅編輯模式）
- **顯示條件**: `selectedMeeting`
- **功能**: 匯出會議通知單文件（Word .docx 格式）
- **圖示**: heroicons:document-arrow-down
- **實作位置**: `basic-info.vue:657-674`
- **API**: `GET /api/meetings/:id/export-notice`
- **詳細說明**: 參見 [會議通知匯出功能說明](./meeting-notice-export.md)
...
3. ✅ 會議日期時間選擇器
4. ✅ 頁面佈局優化
5. ✅ 選擇更新會後自動帶入資料（含理事長姓名）
6. ✅ 排除所有權人不列計功能
7. ✅ 匯出會議通知功能（參見 [meeting-notice-export.md](./meeting-notice-export.md)）
8. ⏳ 匯出簽到冊功能
9. ⏳ 表單驗證提示
10. ⏳ 自動儲存草稿
